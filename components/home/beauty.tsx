...
import { useBeauty, useSkincare } from "@/app/hooks/useBeauty";

export function Beauty() {
    const { data: beautyData, isLoading, error } = useBeauty();
    const { data: skincareData } = useSkincare();
    const allProducts = [
        ...(beautyData ?? []),
        ...(skincareData ?? []),
    ];
        ...
        <Fragment>
            <div className="relative w-full py-4 px-2 mt-5 ">
                <div className="flex justify-between px-2 my-3">
                    <h2 className="text-2xl font-bold text-gray-800 ml-2 pl-3 mb-3 font-sans antialiased ">Beauty picks</h2>
            ...
            hover:bg-gray-50 transition-colors"
                ...
                <div
                    ref={scrollRef}
                    onScroll={onScroll}
                    className="flex gap-4 overflow-x-auto scroll-smooth px-2 scrollbar-none"
                    style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
                >
                    {allProducts?.map((item: any, index: number) => (
                        <button
                            key={index}
                            className="flex flex-col gap-1 flex-shrink-0 group cursor-pointer rounded-lg pb-5 w-[15.68%] border border-gray-200 hover:shadow-md transition-shadow duration-200"
                        >
                            <div className="">
