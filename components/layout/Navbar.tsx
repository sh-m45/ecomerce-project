...
import { ShoppingCart, Home, Briefcase, Package, Search, LogIn, Zap, Heart, User, CircleUserRound, Languages } from "lucide-react";

const Navbar = () => {
  const cartCount = 3;

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center gap-8 px-6 h-16
      border-b border-yellow-300 bg-yellow-300 backdrop-blur-md">

      {/* Brand */}
      <Link href="/"
        className="flex items-center gap-2 font-semibold text-gray-900 shrink-0 w-70">
        <Zap size={20} /> MAAKI
      </Link>
      ...
      {/* Nav links */}
      <div className="flex justify-end-safe w-100  gap-1">
      ...
      {/* Actions */}
      <div className="flex items-center  shrink-0 ml-15 text-gray-700 font-semibold">
        <button
